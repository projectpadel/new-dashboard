-- Block bracket swap when either match already has score / result
...
BEGIN
  ...
  IF v_a.result_locked OR v_b.result_locked
    OR v_a.winner_team_id IS NOT NULL OR v_b.winner_team_id IS NOT NULL
    OR v_a.score_team_a IS NOT NULL OR v_a.score_team_b IS NOT NULL
    OR v_b.score_team_a IS NOT NULL OR v_b.score_team_b IS NOT NULL
    OR lower(coalesce(v_a.status, '')) IN ('completed', 'done')
    OR lower(coalesce(v_b.status, '')) IN ('completed', 'done')
  THEN
    RAISE EXCEPTION 'Tim dari match yang sudah memiliki hasil skor tidak bisa ditukar';
  END IF;
