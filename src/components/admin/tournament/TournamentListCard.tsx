...
export function TournamentListCard({ t }: { t: TournamentListItem }) {
  return (
    <article className="rounded-2xl border bg-card p-5 shadow-sm space-y-4">
      ...
      <div className="flex flex-col items-center text-center gap-2">
        {t.poster_url ? (
          <img src={t.poster_url} alt="" className="h-16 w-16 rounded-full object-cover border" />
        ) : (
      ...
      {t.prize_pool_idr != null && t.prize_pool_idr > 0 && (
        <div className="text-center py-2 rounded-lg bg-muted/40">
          ...
          <p className="text-xl font-bold text-admin-positive tabular-nums">{fmtIDR(t.prize_pool_idr)}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-sm">
        <div>
          ...
          <p className="font-semibold tabular-nums">
            {t.approvedTeamCount}/{t.team_slots}
          </p>
      ...
      <div className="flex gap-2">
        <Button className="flex-1" asChild>
          <Link to="/admin/tournament/$tournamentId" params={{ tournamentId: t.id }}>
            Detail
          </Link>
        </Button>
      </div>
